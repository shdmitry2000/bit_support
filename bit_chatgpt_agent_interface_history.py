...
class Conversational:
    ...
    def __init__(self,vector_db_factory=None,myembeder=Embeddings().getdefaultEmbading()) -> None:
        ...
        if vector_db_factory is None:
            defaulfDBfactoryName=os.getenv("VECTOR_DB_FACTORY",default='vectorDBLlama')
            factory_file_path = os.path.join(os.path.dirname(__file__),  f"{defaulfDBfactoryName}.py" )
            # Load the factory module
            spec = importlib.util.spec_from_file_location(defaulfDBfactoryName, factory_file_path)
            # Load the module dynamically
            vector_db_factory = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(vector_db_factory)


            
        
        logging.info(f"load db factory {vector_db_factory}")
